 ...
 */
void CreateGammaEncodeTable(float gamma, std::vector<uint8_t>& table) {
 ...
 */
void CreateGammaDecodeTable(float gamma, std::vector<uint8_t>&table) {
 ...
 */
bool ApplyGamma(void* dst, void* src, uint32_t w, uint32_t h,
                COLOR_FORMAT format, std::vector<uint8_t>& gammaTable) {
  ASSERT(format == COLOR_FORMAT::R8G8B8A8,
         "not supported format (%d) to gamma function", format);
...
bool TransformColorSpace(IMAGE_FORMAT &dst, IMAGE_FORMAT& src) {
  if (src.format_ != COLOR_FORMAT::R8G8B8A8 ||
      dst.format_ != COLOR_FORMAT::R8G8B8A8 ||
      !src.npm_  || !dst.npm_ ) {
