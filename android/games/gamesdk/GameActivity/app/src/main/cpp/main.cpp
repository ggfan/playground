...
void android_main(struct android_app* state) {
    ...
    while (true) {
        ...
        auto inputBuffer = android_app_swap_input_buffers(state);
        if (inputBuffer) {
                     ...
                     if (motionEvent->pointerCount > 0) {
                         ...
                         if (ptrIndex < motionEvent->pointerCount) {
                            if (actionMasked == AMOTION_EVENT_ACTION_UP ||
                                    ...
                                    engine.animating = 1;
                                    engine.state.x = GameActivityPointerAxes_getX(&motionEvent->pointers[ptrIndex]);
                                    engine.state.y = GameActivityPointerAxes_getY(&motionEvent->pointers[ptrIndex]);
                                    LOGI("MotionEvent: (%d, %d)", engine.state.x, engine.state.y);
                            }
                         }
                     }
                 }
             ...
             if (inputBuffer->keyEventsCount) {
                 LOGI("detected KeyEvent (%d), clearing them", (uint32_t)inputBuffer->keyEventsCount);
